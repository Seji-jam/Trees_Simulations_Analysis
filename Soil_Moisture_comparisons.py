...
for i, file_name in enumerate(file_names):
    ...
    simulated_data['Average_profile']=(simulated_data['theta1'].astype(float)+simulated_data['theta2'].astype(float)+simulated_data['theta3'].astype(float)+simulated_data['theta4'].astype(float))/4
    daily_simulated_data = simulated_data.groupby('Date').agg({'Average_profile': 'mean'}).reset_index()

    # =============================================================================
    # read the measured data
    # =============================================================================
    measurement_file_name = 'Cotton_GoField_SoilWaterContent.xlsx'
    ...
    measured_data_avg = measured_data.groupby([ 'Date']).agg(
        Average_profile=('Average_profile', 'mean')).reset_index()
    measured_data_avg['Average_profile']/=100
    # =============================================================================
    # comparing measured versus simulated
    # =============================================================================
    ax = axes[i]
    
    sns.lineplot(x='Date', y='Average_profile', data=daily_simulated_data, linewidth=2,
                  linestyle='--', color='red', label='Simulated Canopy Temperature', ax=ax)
    
    sns.lineplot(x='Date', y='Average_profile', data=measured_data_avg, linewidth=2,
    ...
    ax.set_xlabel('')
    ax.set_ylabel('Soil Moisture (%)', fontsize=20)
